...
@section('content')
    <div
        x-data="{
            ...
            form: { iku_id: '{{ old('iku_id', '') }}', nama_usulan: '{{ old('nama_usulan', '') }}', deskripsi: '{{ old('deskripsi', '') }}', permasalahan: '{{ old('permasalahan', '') }}' },
        }"
    >
        <div class="flex w-full overflow-hidden rounded-t-2xl bg-white shadow-card">
            ...
            @if ($nextYearAvailable)
                <a href="{{ route('tim-kerja.usulan-program-kerja.index', ['tahun' => 'h_plus_1']) }}"
                   class="flex-1 border-b-2 px-4 py-3 text-center text-sm font-semibold transition-colors
