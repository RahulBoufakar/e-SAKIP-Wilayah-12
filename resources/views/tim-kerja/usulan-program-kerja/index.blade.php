...
@section('subtitle', $tahun ? 'Tahun '.$tahun : 'Belum ada IKU')

@section('content')
    <div
        x-data="{
            modalOpen: {{ $errors->any() ? 'true' : 'false' }},
            form: { iku_id: '{{ old('iku_id', '') }}', nama_usulan: '{{ old('nama_usulan', '') }}', deskripsi: '{{ old('deskripsi', '') }}' },
        }"
    >
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <form method="GET" class="w-full max-w-xs">
                ...
                <select id="tahun" name="tahun" onchange="this.form.submit()"
                        class="w-full rounded-lg border-slate-200 bg-white py-2 pl-3 pr-8 text-sm shadow-card focus:border-brand-500 focus:ring-brand-500">
                    @forelse ($tahunOptions as $t)
                        <option value="{{ $t }}" @selected($t == $tahun)>Tahun {{ $t }}</option>
                    @empty
                        <option value="">Belum ada IKU</option>
                    @endforelse
                </select>
            </form>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($usulanList as $i => $row)
                        <tr class="{{ $loop->even ? 'bg-slate-50/60' : '' }} hover:bg-brand-50/40">
                            ...
                            <td class="max-w-[12rem] px-3 py-2">
                                <x-truncate-cell :id="'deskripsi-'.$row->id" :text="$row->deskripsi ?: '—'" />
                            </td>
                            <td class="px-3 py-2 text-center"><x-status-badge :status="$row->status_validasi" /></td>
                    ...
                    @empty
                        <tr>
                            <td colspan="7" class="px-5 py-12 text-center text-sm text-slate-400">Belum ada Usulan Program Kerja.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
