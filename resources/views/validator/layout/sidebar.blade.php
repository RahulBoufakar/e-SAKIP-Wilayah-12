@php
    ...
    $isTargetKinerja = request()->routeIs('validator.target-kinerja.*', 'validator.iku-lldikti.*', 'validator.rencana-aksi.*');
    $isProgramKerja = request()->routeIs('validator.usulan-program-kerja.*', 'validator.data-proker.*', 'validator.kalender-proker.*', 'validator.pelaporan-kegiatan.*', 'validator.pts-tagging.*');
...
>
    ...
    >
        ...
        <div class="flex min-h-0 flex-1 flex-col bg-white">
            ...
            {{-- TAB ORANGE --}}
            <div class="relative z-10 shrink-0 rounded-t-3xl bg-[#f0a500] py-1.5 text-center text-sm font-bold text-white shadow-sm">
                {{ ucwords(str_replace(['-', '_'], ' ', Auth::user()->getRoleNames()->first() ?? 'Validator')) }}
            </div>

            {{-- CONTAINER MENU BIRU --}}
            <div class="relative z-0 mt-0 flex min-h-0 flex-1 flex-col overflow-hidden rounded-t-none rounded-b-none bg-[#002e5b] pt-2 shadow-lg">
...
>
    ...
    {{-- Tombol toggle ada di Header (navbar) --}}
    ...
    <div class="flex min-h-0 flex-1 flex-col bg-white overflow-hidden">
        ...
        {{-- TAB ORANGE --}}
        <div class="relative z-10 shrink-0 rounded-t-3xl bg-[#f0a500] py-1.5 text-center text-sm font-bold text-white shadow-sm transition-all duration-300 overflow-hidden whitespace-nowrap">
            <span x-show="!desktopCollapsed">{{ ucwords(str_replace(['-', '_'], ' ', Auth::user()->getRoleNames()->first() ?? 'Validator')) }}</span>
        ...
        {{-- CONTAINER MENU BIRU --}}
        <div class="relative z-0 mt-0 flex min-h-0 flex-1 flex-col overflow-hidden rounded-t-none rounded-b-none bg-[#002e5b] pt-2 shadow-lg transition-all duration-300">

            <nav class="min-h-0 flex-1 space-y-1 overflow-y-auto overflow-x-hidden overscroll-contain px-3 py-4 transition-all duration-300">
                @include('validator.layout.partials.sidebar-menu')
            </nav>

            {{-- LOGOUT --}}
            <div class="shrink-0 border-t border-white/10 p-4 transition-all duration-300">
                <form method="POST" action="{{ route('logout') }}" class="w-full">
                    @csrf
                    <button
                        type="submit"
