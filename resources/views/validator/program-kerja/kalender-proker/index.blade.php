...
@section('subtitle', 'Kalender pelaksanaan seluruh Program Kerja yang tervalidasi')

@section('content')
    <div class="flex w-full overflow-hidden rounded-t-2xl bg-white shadow-card">
        <a href="{{ route('validator.kalender-proker.index', ['tahun' => 'berjalan', 'tampilkan_semua' => $tampilkanSemua ? 1 : null]) }}"
        ...
        @if ($nextYearAvailable)
            <a href="{{ route('validator.kalender-proker.index', ['tahun' => 'h_plus_1', 'tampilkan_semua' => $tampilkanSemua ? 1 : null]) }}"
               class="flex-1 border-b-2 px-4 py-3 text-center text-sm font-semibold transition-colors
                                                ...
                                                <ul class="mt-4 space-y-2">
                                                    @foreach ($itemsBulanIku as $item)
                                                        <li class="flex items-center justify-between gap-3 rounded-lg bg-slate-50 px-3 py-2">
                                                            ...
                                                            <a href="{{ route('validator.data-proker.index', ['tahun' => $tab]) }}#proker-{{ $item['id'] }}"
