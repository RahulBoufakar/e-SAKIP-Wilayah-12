<div x-show="modalOpen" x-cloak class="fixed inset-0 z-50 flex items-center justify-center px-4">
    ...
    >
        ...
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-4">
            <h3 class="text-base font-bold text-ink-900">Tambah Jumlah Mahasiswa</h3>
            <button type="button" @click="modalOpen = false" class="text-slate-400 hover:text-slate-600">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
            </button>
        </div>

        <form method="POST" action="{{ route('admin.tools.jumlah-mahasiswa.store') }}" class="flex flex-1 flex-col overflow-hidden">
            @csrf
            ...
            <div class="flex-1 space-y-3 overflow-y-auto px-6 py-4">
                ...
                >
                    ...
                    @foreach ($tahunOptions as $t)
                        <option value="{{ $t->id }}" @selected(old('tahun_anggaran_id') == $t->id)>{{ $t->tahun }}</option>
                    @endforeach
                </x-form.select>
