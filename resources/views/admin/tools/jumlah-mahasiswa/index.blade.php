...
@section('title', 'Setting Jumlah Mahasiswa')
@section('subtitle', 'Data jumlah mahasiswa per tahun anggaran')
...
@section('content')
    <div x-data="{ modalOpen: {{ $errors->any() ? 'true' : 'false' }} }">
            ...
            <table class="w-full text-left text-sm">
                <thead>
                    <tr class="bg-ink-900 text-white">
                        ...
                        <th class="px-5 py-3 font-semibold">Jumlah Mahasiswa</th>
                        ...
                        <tr class="{{ $loop->even ? 'bg-slate-50/60' : '' }} hover:bg-brand-50/40">
                            ...
                            <td class="px-5 py-3">
                                ...
                                @include('admin.layout.confirm-delete', [
                                    ...
                                    'action' => route('admin.tools.jumlah-mahasiswa.destroy', $row->id),
                                    'label' => 'data Jumlah Mahasiswa TA '.$row->tahunAnggaran->tahun,
                                ])
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="3" class="px-5 py-12 text-center text-sm text-slate-400">Belum ada data Jumlah Mahasiswa.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
        ...
        @include('admin.tools.jumlah-mahasiswa._modal')
    </div>
@endsection