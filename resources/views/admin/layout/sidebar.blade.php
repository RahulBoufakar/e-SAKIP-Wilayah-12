@php
    ...
    $isTargetKinerja = request()->routeIs(
        'admin.target-kinerja.*',
        'admin.iku.*',
        'admin.iku-lldikti.*',
        'admin.rencana-aksi.*'
    );
...
>
    ...
    >
        {{-- ... Kode Header & Isi Mobile Sidebar asli Anda (Biarkan sama) ... --}}
    </aside>
</div>
...
{{-- ========================================================= --}}
<aside
    class="hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-30 lg:flex lg:h-[100dvh] lg:flex-col lg:bg-[#f3f4f6] border-r border-gray-200 transition-all duration-300 ease-in-out shadow-sm"
    :class="desktopCollapsed ? 'lg:w-20' : 'lg:w-72'"
>

    {{-- Tombol toggle sudah dipindahkan ke Header (navbar) --}}
    ...
    <div class="flex min-h-0 flex-1 flex-col bg-white overflow-hidden">
        ...
        <div class="relative z-10 shrink-0 rounded-t-3xl bg-[#f0a500] py-1.5 text-center text-sm font-bold text-white shadow-sm transition-all duration-300 overflow-hidden whitespace-nowrap">
            <span x-show="!desktopCollapsed">{{ ucwords(str_replace(['-', '_'], ' ', Auth::user()->getRoleNames()->first() ?? '')) }}</span>
